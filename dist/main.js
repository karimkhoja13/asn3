...
class GreyPig extends Pig {
    constructor(name, breed, height, weight, personality, category, swimmingAbility) {
...
class ChestnutPig extends Pig {
    constructor(name, breed, height, weight, personality, category, language) {
        super(name, breed, height, weight, personality, category); // call the super class constructor
...
class WhitePig extends Pig {
    constructor(name, breed, height, weight, personality, category, runningAbility) {
        super(name, breed, height, weight, personality, category); // call the super class constructor
...
class BlackPig extends Pig {
    constructor(name, breed, height, weight, personality, category, strengthAbility) {
        super(name, breed, height, weight, personality, category); // call the super class constructor
...
let pigTable = document.getElementById('pig-table');
let moreInfoTable = document.getElementById('more-info');
let pigDetailsSection = document.getElementById('pig_details');
...
let personalityInput = document.getElementById('personality-input');
let uniqueAttribute = document.getElementById('unique-attribute');
...
submitButton.addEventListener('click', () => {
    ...
    let newPig;
    if (category === 'Grey') {
        let swimmingAbility = document.getElementById('swimming-input').valueAsNumber;
        if (swimmingAbility < 0 ||
            swimmingAbility > 100 ||
        ...
        newPig = new GreyPig(name, breed, height, weight, personality, 'Grey Pig', swimmingAbility);
    }
    else if (category === 'Chestnut') {
        ...
        newPig = new ChestnutPig(name, breed, height, weight, personality, 'Chestnut Pig', language);
    }
    else if (category === 'White') {
        let runningAbility = document.getElementById('running-input').valueAsNumber;
        if (runningAbility < 0 || runningAbility > 100 || isNaN(runningAbility)) {
            errorMsg.innerHTML =
                'Please enter a running ability value between 0 and 100';
            return;
        }
        newPig = new WhitePig(name, breed, height, weight, personality, 'White Pig', runningAbility);
    }
    else if (category === 'Black') {
        let strength = document.getElementById('strength-input').valueAsNumber;
        if (strength < 1 || strength > 10 || isNaN(strength)) {
            errorMsg.innerHTML =
                'Please enter a strength ability value between 1 and 10';
            return;
        }
        newPig = new BlackPig(name, breed, height, weight, personality, 'Black Pig', strength);
    }
    pigs.push(newPig);
    localStorage.setItem('pigs', JSON.stringify(pigs));
    populateTable();
